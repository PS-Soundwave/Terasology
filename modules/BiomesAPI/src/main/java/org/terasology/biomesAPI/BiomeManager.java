...
public class BiomeManager extends BaseComponentSystem implements BiomeRegistry {
    ...
    public void checkBiomeChangeEvent(MovedEvent event, EntityRef entity) {
        final Vector3i newPosition = new Vector3i(JomlUtil.from(event.getPosition()), RoundingMode.FLOOR);
        final Vector3i oldPosition = new Vector3i(JomlUtil.from(event.getPosition().sub(event.getDelta())), RoundingMode.FLOOR);
